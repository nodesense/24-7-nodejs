...
function init_cassandra(){
	...
	client.connect()
		...
		.then(function () {
			const query = `CREATE TABLE IF NOT EXISTS ${KEYSPACE_NAME}.brands 
				 (id uuid, name text,  email text, phone text, PRIMARY KEY (id))`;
			return client.execute(query);
		})
		.then(function () {
			return client.metadata.getTable(KEYSPACE_NAME, 'brands');
		})
		.then(function (table) {
